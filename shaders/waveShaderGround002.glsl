...
vec4 parabolicHatch( vec2 mousePos, vec2 animateVec, vec2 pos, float lineWidth, vec2 colorVecA,  float time, int hatchDensity ){
    ...
    float r = random(colorVecA, colorVecB)/(color_val_r*sin(ang)*2.0);
    float g = random(colorVecA, colorVecB)/(color_val_g*sin(ang)*2.0);
    float b = random(colorVecA, colorVecB)/(color_val_b*sin(ang)*2.0);
    float  a = 1.0;

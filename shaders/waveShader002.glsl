...
vec4 parabolicHatch( vec2 mousePos, vec2 animateVec, vec2 pos, float lineWidth, vec2 colorVecA,  float time, int hatchDensity ){
    ...
    float r = random(colorVecA, colorVecB)/(color_val_r *sin(ang));
    float g = random(colorVecA, colorVecB)/(color_val_g*sin(ang));
    float b = random(colorVecA, colorVecB)/(color_val_b*sin(ang));
    float  a = 1.0;

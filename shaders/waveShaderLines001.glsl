...
float random (vec2 pos, vec2 pos2){
    ...
    return fract(cos(dot(pos.xy, pos2))*999999.78);
...
vec4 parabolicHatch( vec2 mousePos, vec2 animateVec, vec2 pos, float lineWidth, vec2 colorVecA,  float time, int hatchDensity ){
    ...
    float r = random(colorVecA, colorVecB)/.5;
    float g = random(colorVecA, colorVecB)/.5;
    float b = random(colorVecA, colorVecB)/.5;
    float  a = 1.0;
...
vec2 pos = gl_FragCoord.xy/(u_resolution.xy *50000000000000000000.0);
...
gl_FragColor = parabolicHatch( mouseVec, animVec, pos, 1000.0,colVecA,  u_time, hatch_density);

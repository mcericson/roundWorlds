...
vec4 parabolicHatch( vec2 mousePos, vec2 animateVec, vec2 pos, float lineWidth, vec2 colorVecA,  float time, int hatchDensity ){
    ...
    vec2 colorVecB = vec2(pointColor(pos), colorVecA.y);
 
    float r = random(colorVecA, colorVecB)/1.5;
    float g = random(colorVecA, colorVecB)/1.4;
    float b = random(colorVecA, colorVecB)/1.0;
    float  a = 1.0;
...
gl_FragColor = parabolicHatch( mouseVec, animVec, pos, .0009,colVecA,  u_time, hatch_density );

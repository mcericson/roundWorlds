...
float random (vec2 pos, vec2 pos2){
    ...
    return fract(cos(dot(pos.xy, pos2))*1000.0);
}
...
void main() {
    ...
    float hatch_angle = radians(u_time/100.0);
    ...
    vec2 pos2 = vec2(pointColor(pos.xy), pos.x);
    float r  = pos.y/4.0;//fract(sin(dot(pos.xy, pos2)))/3.0;//255.0;
    float g = random(pos2, pos)/6.0;
    float b = random(pos, pos2)*pos.y;//fract(sin(dot(pos.xy, pos2)));
    float  a = 1.0;

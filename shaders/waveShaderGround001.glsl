...
float random (vec2 pos, vec2 pos2){
    ...
    return fract(cos(dot(pos.xy, pos2))*58700.78);
...
vec4 parabolicHatch( vec2 mousePos, vec2 animateVec, vec2 pos, float lineWidth, vec2 colorVecA,  float time, int hatchDensity ){
    ...
    float r = random(colorVecA, colorVecB)/color_val_red_ground;
    float g = random(colorVecA, colorVecB)/color_val_green_ground;
    float b = random(colorVecA, colorVecB)/color_val_blue_ground;
    float  a = 1.0;
...
gl_FragColor = parabolicHatch( mouseVec, animVec, pos, .0001,colVecA,  u_time, hatch_density );
